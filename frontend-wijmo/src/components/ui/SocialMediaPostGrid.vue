<template>
    <div style="max-height:80vh;">
        ...
        <wj-flex-grid
            ...
            class="wj-felx-grid"
        >
            <wj-flex-grid-filter :filterColumns="['RowHeader','postId','content','imageUrl',]" />
            <wj-flex-grid-cell-template cellType="RowHeader" v-slot="cell">{{cell.row.index + 1}}</wj-flex-grid-cell-template>
            <wj-flex-grid-column binding="postId" header="PostId" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="content" header="Content" width="2*" :isReadOnly="true" align="center" />
            <wj-flex-grid-column binding="imageUrl" header="ImageUrl" width="2*" :isReadOnly="true" align="center" />
        </wj-flex-grid>
        <v-col>
            <v-dialog
                v-model="openDialog"
                ...
                <template>
                    <v-card>
                        <v-toolbar
                            ...
                            height="50px"
                        >
                            <div style="color:white; font-size:17px; font-weight:700;">소셜 미디어 포스트 등록</div>
                            <v-spacer></v-spacer>
                            <v-icon
                                color="white"
                                small
                        ...
                        <v-card-text>
                            <SocialMediaPost :offline="offline"
                                :isNew="!itemToEdit"
                                :editMode="true"
                                v-model="newValue"
                                @add="append"
